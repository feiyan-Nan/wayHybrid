  ...
  <div class="activy-root">
    ...
    <div class="conten" v-else>
      ...
      >
        ...
        <title-and-info :info="info"></title-and-info>
        <div class="pd20">
          <div class="line"></div>
        </div>
        <template v-if="eventDesc">
          <div class="sub-title mr20">派对介绍</div>
          <div class="pd20">
        ...
        <!-- 用户信息和关注按钮 -->
        <p class="sub-title mr20">派对邀请人</p>
        <div class="user-info">
        ...
        <template v-if="loadList.length > 0">
          <div class="sub-title pdlr20">派对记忆</div>
      ...
      <div class="footer" @click="openApp">{{eventStatus == 1 ? '报名' : '晒现场'}}</div>
    </div>
  </div>
</template>
...
import { getEventDetailApi, getContListApi } from '@/api/activity'
      ...
      getEventDetailApi(obj)
        .then(res => {
          ...
          if (res.data.code == 2000) {
            ...
            this.eventDescArray = res.data.data.eventDesc.split('#')
